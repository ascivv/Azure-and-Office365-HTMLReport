...
$CompanyLogo = "C:\PowerShell\hcfulllogo.png"

#Location the report will be saved to
$ReportSavePath = "C:\PowerShell"
...
{
	$MFAStatus = get-msoluser -objectid $Admin.ObjectID | select @{N="MFA Status"; E={ if( $_.StrongAuthenticationRequirements.State -ne $null)
...
$exchrole = Get-AzureADDirectoryRole | Where-Object { $_.DisplayName -match "Exchange Service Administrator" }
$ExchAdmins = Get-AzureADDirectoryRoleMember -ObjectId $exchrole.ObjectId
Foreach ($ExchAdmin in $ExchAdmins)
{
	$MFAStatus = get-msoluser -objectid $ExchAdmin.ObjectID | select @{N="MFA Status"; E={ if( $_.StrongAuthenticationRequirements.State -ne $null)
		{ $_.StrongAuthenticationRequirements.State} 
		else { "Disabled"}}}
...
$privadminrole = Get-AzureADDirectoryRole | Where-Object { $_.DisplayName -match "Privileged Role Administrator" }
$PrivAdmins = Get-AzureADDirectoryRoleMember -ObjectId $privadminrole.ObjectId
Foreach ($PrivAdmin in $PrivAdmins)
{
	$MFAStatus = get-msoluser -objectid $PrivAdmin.ObjectID | select @{N="MFA Status"; E={ if( $_.StrongAuthenticationRequirements.State -ne $null)
		{ $_.StrongAuthenticationRequirements.State} 
		else { "Disabled"}}}
...
$userrole = Get-AzureADDirectoryRole | Where-Object { $_.DisplayName -match "User Account Administrator" }
$userAdmins = Get-AzureADDirectoryRoleMember -ObjectId $userrole.ObjectId
Foreach ($userAdmin in $userAdmins)
{
	$MFAStatus = get-msoluser -objectid $userAdmin.ObjectID | select @{N="MFA Status"; E={ if( $_.StrongAuthenticationRequirements.State -ne $null)
		{ $_.StrongAuthenticationRequirements.State} 
		else { "Disabled"}}}
...
$TechExchAdmins = Get-RoleGroupMember -Identity "Tech Account Restricted Exchange Management"
Foreach ($TechExchAdmin in $TechExchAdmins)
{
	$AccountInfo = get-msoluser -searchstring $TechExchAdmin.Name
	$Name = $AccountInfo.DisplayName
	$MFAStatus = get-msoluser -objectid $AccountInfo.ObjectID | select @{N="MFA Status"; E={ if( $_.StrongAuthenticationRequirements.State -ne $null)
		{ $_.StrongAuthenticationRequirements.State} 
		else { "Disabled"}}}
...
$sprole = Get-AzureADDirectoryRole | Where-Object { $_.DisplayName -match "SharePoint Service Administrator" }
$SPAdmins = Get-AzureADDirectoryRoleMember -ObjectId $sprole.ObjectId
Foreach ($SPAdmin in $SPAdmins)
{
	$MFAStatus = get-msoluser -objectid $SPAdmin.ObjectID | select @{N="MFA Status"; E={ if( $_.StrongAuthenticationRequirements.State -ne $null)
		{ $_.StrongAuthenticationRequirements.State} 
		else { "Disabled"}}}
...
$skyperole = Get-AzureADDirectoryRole | Where-Object { $_.DisplayName -match "Lync Service Administrator" }
$skypeAdmins = Get-AzureADDirectoryRoleMember -ObjectId $skyperole.ObjectId
Foreach ($skypeAdmin in $skypeAdmins)
{
	$MFAStatus = get-msoluser -objectid $skypeAdmin.ObjectID | select @{N="MFA Status"; E={ if( $_.StrongAuthenticationRequirements.State -ne $null)
		{ $_.StrongAuthenticationRequirements.State} 
		else { "Disabled"}}}
...
$crmrole = Get-AzureADDirectoryRole | Where-Object { $_.DisplayName -match "CRM Service Administrator" }
$crmAdmins = Get-AzureADDirectoryRoleMember -ObjectId $crmrole.ObjectId
Foreach ($crmAdmin in $crmAdmins)
{
	$MFAStatus = get-msoluser -objectid $crmAdmin.ObjectID | select @{N="MFA Status"; E={ if( $_.StrongAuthenticationRequirements.State -ne $null)
		{ $_.StrongAuthenticationRequirements.State} 
		else { "Disabled"}}}
...
$birole = Get-AzureADDirectoryRole | Where-Object { $_.DisplayName -match "Power BI Service Administrator" }
$biAdmins = Get-AzureADDirectoryRoleMember -ObjectId $birole.ObjectId
Foreach ($biAdmin in $biAdmins)
{
	$MFAStatus = get-msoluser -objectid $biAdmin.ObjectID | select @{N="MFA Status"; E={ if( $_.StrongAuthenticationRequirements.State -ne $null)
		{ $_.StrongAuthenticationRequirements.State} 
		else { "Disabled"}}}
...
$servicerole = Get-AzureADDirectoryRole | Where-Object { $_.DisplayName -match "Service Support Administrator" }
$serviceAdmins = Get-AzureADDirectoryRoleMember -ObjectId $servicerole.ObjectId
Foreach ($serviceAdmin in $serviceAdmins)
{
	$MFAStatus = get-msoluser -objectid $serviceAdmin.ObjectID | select @{N="MFA Status"; E={ if( $_.StrongAuthenticationRequirements.State -ne $null)
		{ $_.StrongAuthenticationRequirements.State} 
		else { "Disabled"}}}
...
$billingrole = Get-AzureADDirectoryRole | Where-Object { $_.DisplayName -match "Billing Administrator" }
$billingAdmins = Get-AzureADDirectoryRoleMember -ObjectId $billingrole.ObjectId
Foreach ($billingAdmin in $billingAdmins)
{
	$MFAStatus = get-msoluser -objectid $billingAdmin.ObjectID | select @{N="MFA Status"; E={ if( $_.StrongAuthenticationRequirements.State -ne $null)
		{ $_.StrongAuthenticationRequirements.State} 
		else { "Disabled"}}}
